    ...
    loop {
        ...
        if is_key_pressed(KeyCode::N) {
            field.spawn_triball(
                30.0,
                (
                    macroquad::rand::gen_range::<f32>(
                        screen_width() / 2.0 - field.get_size() / 2.0 + 15.0,
                        screen_width() / 2.0 + field.get_size() / 2.0 - 15.0,
                    ),
                    macroquad::rand::gen_range::<f32>(
                        screen_height() / 2.0 - field.get_size() / 2.0 + 15.0,
                        screen_height() / 2.0 + field.get_size() / 2.0 - 15.0,
                    ),
                ),
            );
        ...
        match ui.state() {
            ...
            ui::State::Auton => {
                if !autonStarted {
                    autonStarted = true;
                    thread::spawn(move || {
                        // movement::pidMTP(&robot, (width / 2.0, height / 2.0), 115.0, 800, L_CONS, R_CONS, 10.0);
                        // movement::boomerang(&robot, (width - width / 8.0, height - height/ 6.0), 40000, 0.7, PI/2.0, 90.0, L_CONS, R_CONS, 0.0);
                        movement::boomerang(
                            &robot,
                        ...
                        // movement::eulerTurn(&robot, 0.0, -0.0002, movement::eulerTurn(&robot, 45.0, 0.0002, 0.0, 3000, 1, TURN_CONS), 3000, -1, TURN_CONS);
                        ...
                        // movement::followPath(&robot, path, 100000, 0.7, PI/2.0, 90.0, L_CONS, R_CONS);
                    });
                }
            }
        }
...
const L_CONS: util::PidConstants = util::PidConstants {
    p: 0.03,
    i: 0.0,
    d: 0.0,
    tolerance: 0.0,
...
const R_CONS: util::PidConstants = util::PidConstants {
    p: 0.015,
    i: 0.0,
    d: 0.0,
    tolerance: 0.0,
...
const TURN_CONS: util::PidConstants = util::PidConstants {
    p: 0.024,
    i: 0.0,
    d: 0.0,
    tolerance: 0.0,
